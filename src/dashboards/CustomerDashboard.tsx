      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ 
              background: ROLE_COLORS[user?.role || 'CUSTOMER'], color: '#fff', 
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
          <div style={{ marginBottom: '40px' }}>
            <h1 style={{ fontSize: '32px', fontWeight: 700, marginBottom: '8px' }}>Customer Dashboard</h1>
            <p className="text-muted" style={{ fontSize: '16px' }}>Welcome back, {user?.name || 'Customer'} 👋</p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', marginBottom: '40px' }}>
            {[
              { icon: '🛍️', label: 'Total Orders' },
              { icon: '⭐', label: 'Reviews Given' },
              { icon: '🎟️', label: 'Active Promos' },
              { icon: '💖', label: 'Favorite Dishes' }
            ].map((stat, idx) => (
