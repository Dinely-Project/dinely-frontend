...
const StaffDashboard: React.FC = () => {
  ...
  /** Non-null when staff has clicked "View Details" on an order card */
  ...
  const handleNavClick = (label: ActiveSection) => {
    ...
    setDetailOrderId(null); // reset detail view when switching sections
  };
  ...
  const renderMain = () => {
    ...
    if (activeSection === 'Orders') {
      ...
      return (
        <LiveOrdersPanel onViewDetail={(id) => setDetailOrderId(id)} />
      );
    }

    // Overview (and unimplemented stubs)
    return (
      <div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a0a', color: '#fff' }}>
      ...
      <header
        style={{
          height: '70px', padding: '0 24px', display: 'flex', justifyContent: 'space-between',
          ...
          {navItems.map((item) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer', fontWeight: activeSection === item.label ? 600 : 500,
                transition: 'all 0.2s', border: 'none', textAlign: 'left',
